...
public class MoGuHttpClient {
	...
	public String uploadAvatar(String strUrl, byte[] bytes, String userID) {
		...
		try {
			...
			for (int i = 0; i < leng; i++) {
				...
				sb.append(BOUNDARY);
				sb.append("\r\n");
				sb.append("Content-Disposition: form-data;name=\"user_avatar" + "\";filename=\"" + file.getName() + "\"\r\n");
				sb.append("Content-Type:application/octet-stream\r\n\r\n");

				byte[] image = sb.toString().getBytes();
				out.write(image);
				out.write(bytes);
				out.write("\r\n".getBytes()); // 多个文件时，二个文件之间加入这个
			}
	...
	public String uploadFeedback(String strUrl, byte[] content, List<byte[]> bytesList, String userID) {
		List<String> list = new ArrayList<String>(); // 要上传的文件名,如：d:\haha.doc.你要实现自己的业务。我这里就是一个空list.
        String comment = "comment";
		String feedbackPic1 = "feedback_pic1";
		String feedbackPic2 = "feedback_pic2";
		String feedbackPic3 = "feedback_pic3";
		list.add(comment);
		list.add(feedbackPic1);
		list.add(feedbackPic2);
		list.add(feedbackPic3);
        List<byte[]> fileList = new ArrayList<>();
		try {
            fileList.add(content);
			if (bytesList.size() > 0) {
				fileList.addAll(bytesList);
			}
			String BOUNDARY = "---------7d4a6d158c9"; // 定义数据分隔线
			URL url = new URL(strUrl);
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			...
			for (int i = 0; i < leng; i++) {
				...
				sb.append(BOUNDARY);
				sb.append("\r\n");
				sb.append("Content-Disposition: form-data;name=\"" + fname + "\";filename=\"" + fname + "\"\r\n");
				sb.append("Content-Type:application/octet-stream\r\n\r\n");
			...
			Log.e("yuki", "pic#发送POST请求出现异常！" + e.toString());
			e.printStackTrace();
			return "ok";
		}
	}

    public void uploadApkLog(String strUrl, String strCode, String strContent, String strUserID) {
        try {
            String BOUNDARY = "---------7d4a6d158c9"; // 定义数据分隔线
            URL url = new URL(strUrl);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            ...
            sb.append(BOUNDARY);
            sb.append("\r\n");
            sb.append("Content-Disposition: form-data; name=\"event_id\"");
            sb.append("\r\n\r\n");
            sb.append(strCode);
            sb.append("\r\n");
            //sb.append("Content-Type:application/octet-stream\r\n\r\n");
            byte[] bcode = sb.toString().getBytes();
            out.write(bcode);
            ...
            sb.append(BOUNDARY);
            sb.append("\r\n");
            sb.append("Content-Disposition: form-data; name=\"event_msg\"");
            sb.append("\r\n\r\n");
            sb.append(strContent);
            sb.append("\r\n");
            //sb.append("Content-Type:application/octet-stream\r\n\r\n");
            byte[] bcontent = sb.toString().getBytes();
            out.write(bcontent);
