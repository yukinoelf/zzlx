package com.zhizulx.tt.ui.widget.swiprecycleview;
...
public class RecyItemTouchHelperCallback extends ItemTouchHelper.Callback {
    ...
    public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, RecyclerView.ViewHolder target) {
        ...
        if (fromPosition < toPosition) {
            for (int i = fromPosition; i < toPosition; i++) {
                Collections.swap(((CollectionAdapter) mAdapter).getDataList(), i, i + 1);
            }
        } else {
            for (int i = fromPosition; i > toPosition; i--) {
                Collections.swap(((CollectionAdapter) mAdapter).getDataList(), i, i - 1);
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        ((CollectionAdapter) mAdapter).getDataList().remove(adapterPosition);
    }
