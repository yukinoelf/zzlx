...
void CDBServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        ...
        case CID_BUDDY_LIST_TRAVEL_CREATE_RESPONSE:
            ...
            break;
        case CID_BUDDY_LIST_RADOM_ROUTE_QUERY_RESPONSE:
            _HandleQueryRadomRouteResponse(pPdu);
            break;
        case CID_BUDDY_LIST_RADOM_ROUTE_UPDATE_RESPONSE:
            _HandleUpdateRadomRouteResponse(pPdu);
            break;
        case CID_BUDDY_LIST_NEW_CREATE_COLLECT_ROUTE_RESPONSE:
            _HandleCreateCollectRouteResponse(pPdu);
            break;
        case CID_BUDDY_LIST_NEW_DELETE_COLLECT_ROUTE_RESPONSE:
            _HandleDeleteCollectRouteResponse(pPdu);
            break;
        case CID_BUDDY_LIST_NEW_QUERY_COLLECT_ROUTE_RESPONSE:
            _HandleQueryCollectRouteResponse(pPdu);
            break;
        case CID_BUDDY_LIST_NEW_TRAVEL_CREATE_RESPONSE:
            _HandleNewCreateTravelResponse(pPdu);
            break;
        case CID_BUDDY_LIST_TRAVEL_UPDATE_RESPONSE:
            _HandleUpdateTravelResponse(pPdu);
            break;
        case CID_BUDDY_LIST_TRAVEL_DELETE_RESPONSE:
            _HandleDeleteTravelResponse(pPdu);
            break;
        ...
        case CID_FILE_HAS_OFFLINE_RES:
            ...
            break;
        case CID_BUDDY_INFO_MODIFY_RESPONSE:
        	_HandleInfoModifyRsp(pPdu);
        	break;
...
void CDBServConn::_HandleQueryRadomRouteResponse(CImPdu* pPdu) {
    IM::Buddy::NewQueryRadomRouteRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleUpdateRadomRouteResponse(CImPdu* pPdu) {
    IM::Buddy::NewUpdateRadomRouteRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleNewCreateTravelResponse(CImPdu* pPdu) {
    IM::Buddy::NewCreateMyTravelRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleUpdateTravelResponse(CImPdu* pPdu) {
    IM::Buddy::UpdateMyTravelRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleDeleteTravelResponse(CImPdu* pPdu) {
    ...
    uint32_t user_id = msg.user_id();
    uint32_t result = msg.result_code();    
    log("_HandleDeleteTravelResponse: user_id=%u, result=%u.", user_id, result);
    ...
    if (pMsgConn && pMsgConn->IsOpen()) {
        ...
        log("_HandleDeleteTravelResponse: can't found msg_conn by user_id = %u, handle = %u", user_id, handle);
    }
}
...
void CDBServConn::_HandleCreateCollectRouteResponse(CImPdu* pPdu) {
    IM::Buddy::NewCreateCollectRouteRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleDeleteCollectRouteResponse(CImPdu* pPdu) {
    IM::Buddy::NewDelCollectRouteRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleQueryCollectRouteResponse(CImPdu* pPdu) {
    IM::Buddy::NewQueryCollectRouteRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleInfoModifyRsp(CImPdu* pPdu) {
	IM::Buddy::Info_Modify_Rsp msg;
	msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength());
	CDbAttachData attach_data((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());
	CMsgConn* pMsgConn = CImUserManager::GetInstance()->GetMsgConnByHandle(msg.user_id(), attach_data.GetHandle());
	if (pMsgConn && pMsgConn->IsOpen())
	{
		pPdu->SetPBMsg(&msg);
		pMsgConn->SendPdu(pPdu);
